...
<template>
  <div>
    <Button icon="pi pi-user-plus" @click="isOpen = true" />
    <Dialog
    ...
    >
      <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
        <div class="flex flex-col gap-4">
          <div>
            <label for="acc-no" class="block mb-1">Account Number</label>
            <InputNumber
              id="acc-no"
          ...
          <div>
            <label for="name" class="block mb-1">Firstname</label>
            <InputText
              id="name"
        ...
        <div class="w-full flex items-center justify-end gap-4">
          <Button
            ...
            @click="isOpen = false"
          />
          <Button label="Save" :loading="isLoading" type="submit" />
        </div>
      </form>
    </Dialog>
  </div>
