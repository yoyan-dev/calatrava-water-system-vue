<script setup lang="ts">
	...
	onMounted(async () => {
		...
		console.log('Resident data fetched:', store.resident);
		if (store.resident?.billings && store.resident?.billings?.length > 1) {
			currentBill.value = Number(store.resident?.billings[0].totalBill) || 0;
			previousBill.value = Number(store.resident?.billings[1].totalBill) || 0;
						...
						<div
							...
							<div>
								<span class="text-xl font-bold text-surface-200">{{
									formatToPeso(store.resident?.billings?.[0].totalBill || 0)
								}}</span
								><br />
								<div>
									<div
